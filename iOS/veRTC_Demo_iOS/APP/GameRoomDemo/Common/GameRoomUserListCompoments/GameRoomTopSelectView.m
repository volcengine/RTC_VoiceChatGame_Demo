...
@interface GameRoomTopSelectView ()
...
@implementation GameRoomTopSelectView
...
- (void)raiseButtonAction {
    ...
    if ([self.delegate respondsToSelector:@selector(gameRoomTopSelectView:clickSwitchItem:)]) {
        [self.delegate gameRoomTopSelectView:self clickSwitchItem:NO];
    }
}
...
- (void)audienceButtonAction {
    ...
    if ([self.delegate respondsToSelector:@selector(gameRoomTopSelectView:clickSwitchItem:)]) {
        [self.delegate gameRoomTopSelectView:self clickSwitchItem:YES];
    }
}

- (void)cancelButtonAction {
    if ([self.delegate respondsToSelector:@selector(gameRoomTopSelectView:clickCancelAction:)]) {
        [self.delegate gameRoomTopSelectView:self clickCancelAction:@""];
    }
}
...
- (BaseButton *)cancelButton {
    if (!_cancelButton) {
        ...
        [_cancelButton setImage:[UIImage imageNamed:@"voice_cancel" bundleName:HomeBundleName] forState:UIControlStateNormal];
