...
@interface GameEndComponents ()

@property (nonatomic, strong) GameRoomEndView *endView;
@property (nonatomic, strong) UIButton *maskView;

@end

@implementation GameEndComponents
...
- (void)showWithStatus:(GameEndStatus)status {
    ...
    self.endView.gameEndStatus = status;
    __weak __typeof(self) wself = self;
    self.endView.clickButtonBlock = ^(GameButtonStatus status) {
...
- (GameRoomEndView *)endView {
    if (!_endView) {
        _endView = [[GameRoomEndView alloc] init];
