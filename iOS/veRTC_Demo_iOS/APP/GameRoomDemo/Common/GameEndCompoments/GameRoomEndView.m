...
@implementation GameRoomEndView
...
- (void)endVoiceButtonAction {
    if (self.clickButtonBlock) {
        self.clickButtonBlock(GameButtonStatusEnd);
    }
}

- (void)leaveVoiceButtonAction {
    if (self.clickButtonBlock) {
        self.clickButtonBlock(GameButtonStatusLeave);
    }
}

- (void)cancelVoiceButtonAction {
    if (self.clickButtonBlock) {
        self.clickButtonBlock(GameButtonStatusCancel);
    }
}

- (void)setGameEndStatus:(GameEndStatus)gameEndStatus {
    _gameEndStatus = gameEndStatus;
    ...
    [self updateButtonColorWithType:GameButtonColorTypeRemind button:self.leaveButton];
    [self updateButtonColorWithType:GameButtonColorTypeNone button:self.cancelButton];
    [viewLists addObject:self.leaveButton];
    [viewLists addObject:self.cancelButton];
    
    if (gameEndStatus == GameEndStatusHostOnly) {
        self.titleLabel.text = @"离开会解散房间，是否确认离开？";
    } else if (gameEndStatus == GameEndStatusHost) {
...
- (void)updateButtonColorWithType:(GameButtonColorType)type button:(UIButton *)button {
    button.enabled = YES;
    if (type == GameButtonColorTypeRemind) {
